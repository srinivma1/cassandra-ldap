...
public class LDAPAuthenticator implements IAuthenticator
{
    ...
    {
    	System.out.println("Inside getUid and user is" +user);
    ...
    private String authDN(User user) throws LDAPAuthFailedException {
    	System.out.println("Inside auth DN");
    ...
    {
        String dn;
        System.out.println("Inside authenticate method" + username);
        try
        {
            dn = getUid(username);
            if (dn == null)
            ...
            logger.info("DN for user {}: {}", username, dn);
            ...
            if (cachedPass != null)
            {
                ...
                if(serviceDN.indexOf(username) >0) {
                if (!userExists(dn))
                {
                    logger.info("DN {} doesn't exist in {}.{}, creating new user", dn, SchemaConstants.AUTH_KEYSPACE_NAME, AuthKeyspace.ROLES);
                ...
                return new AuthenticatedUser(dn);
                    ...
                    {
                        logger.info("User {} doesn't exist in {}.{}, creating new user", username, SchemaConstants.AUTH_KEYSPACE_NAME, AuthKeyspace.ROLES);
                        System.out.println("User does not exist");
                        //createRole(dn);
                    }
    ...
    {
    	System.out.println("Inside userExist method");
